...
function App() {
  ...
  useEffect(() => {
    // Em produção (Vercel), usa o proxy /api/products que aplica Edge Cache de 5 min.
    // Em desenvolvimento local, cai direto na URL do Apps Script via variável de ambiente.
    const isVercelProd =
      typeof window !== "undefined" && window.location.hostname !== "localhost";
    const productsUrl = isVercelProd
      ? "/api/products"
      : import.meta.env.VITE_PRODUCTS_SCRIPT_URL ||
      ...
      fetch(productsUrl)
        ...
        .then((data) => {
          if (Array.isArray(data)) {
            ...
            console.error(
              "Erro na resposta da planilha (não é uma lista):",
              data,
            );
        ...
        .catch((err) => {
          console.error(
            "Erro de rede ao buscar produtos. Usando fallback local.",
            err,
          );
          import("./data/products").then((module) =>
            setProducts(module.products),
        ...
        >
          <div className="text-center mb-16 space-y-4">
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Explore nossa coleção exclusiva desenhada para expressar sua fé e
              identidade.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {isLoadingProducts ? (
              ...
              [1, 2, 3, 4].map((idx) => (
                <div
                  key={idx}
                  className="bg-white rounded-xl shadow-sm h-96 animate-pulse border border-gray-100 flex flex-col"
                >
